/**
*   @file       a94b114_usart.c
*   @brief      a94b114 usart peripheral function 
...
void USART_Initial(uint32_t speed, uint8_t data_length, uint8_t stop_bits, uint8_t parity, uint8_t mode)
{
	...
	//temp_reg2 |= (0x1 << 0); //if using Double Speed operation, Enable UCTRL2 |= (0x1 << 0)
	if(temp_reg2 & 0x1) //Double Speed operation(only UART)
	{
		buadrate_val = ((Clock_GetSystemFreq() / speed )  >> 3) * 10;
	}
	else if(temp_reg & 0x40) //Synchronous mode(USART)
	{
...
void USART_ReceiveDataWithPolling(uint8_t *receive_data, uint8_t count)
{
	...
	for(i = 0; i < count; i++)
	{
		while(( USTAT & (1 << 5))!= (1 << 5));		// Wait Data in
...
{	
	...
	if ( USART_rx_queue_is_full () )
	{
		...
		{
			USART_rx_queue [ USART_rx_rear ] = UDATA;
...
void USART_TXInt_Handler() interrupt USART_TX_VECT
{
	...
	{
		UDATA = USART_tx_queue [ USART_tx_front ];
